...
fb22_vars2 <- fread("fb_2022_adid_text.csv.gz", data.table = F)
# "fb_2022_adid_text.csv.gz" is the output table from
# part of data-post-production repo that merges preprocessed results.
# Source: data-post-production/01-merge-results/01_merge_preprocessed_results
...
ngoals <- fb22 %>%
  select(DONATE:PRIMARY_PERSUADE) %>%
  apply(., 1, sum)
...
topspenders <- data.frame(
  spend = goal_by_sponsor %>% select(DONATE:PRIMARY_PERSUADE) %>% apply(., 1, sum),
...
ggplot(goal_by_topsponsor, aes(Spend, `Page Name`)) +
  geom_col(aes(fill = Goal)) +
...
for (i in 1:length(goals)) {
  ...
  ggplot(top_sponsors_by_goal, aes(Spend, `Page Name`)) +
    geom_col() +
    theme(legend.position = "bottom") +
    labs(title = goals[i])
...
ggplot(fb22_age, aes(age)) +
  geom_density()
ggplot(fb22_age[fb22_age$DONATE == 1, ], aes(age)) +
  geom_density()
ggplot(fb22_age[fb22_age$CONTACT == 1, ], aes(age)) +
  geom_density()
ggplot(fb22_age[fb22_age$PURCHASE == 1, ], aes(age)) +
  geom_density()
ggplot(fb22_age[fb22_age$GOTV == 1, ], aes(age)) +
  geom_density()
ggplot(fb22_age[fb22_age$EVENT == 1, ], aes(age)) +
  geom_density()
ggplot(fb22_age[fb22_age$POLL == 1, ], aes(age)) +
  geom_density()
ggplot(fb22_age[fb22_age$GATHERINFO == 1, ], aes(age)) +
  geom_density()
ggplot(fb22_age[fb22_age$LEARNMORE == 1, ], aes(age)) +
  geom_density()
ggplot(fb22_age[fb22_age$PRIMARY_PERSUADE == 1, ], aes(age)) +
  geom_density()
...
fb22_age_df <- fb22_age %>%
  select(DONATE:PRIMARY_PERSUADE) %>%
  as.matrix(.) * fb22_age$age
...
ggplot(fb22_age_df, aes(Age)) +
  ...
  stat_summary(aes(label = ..x.., y = 0), fun = function(x) {
    round(mean(x), 2)
  }, geom = "text", orientation = "y", color = "darkgray", hjust = -0.2) +
  facet_wrap(~Goal) +
  theme_bw() +
  labs(y = "")
